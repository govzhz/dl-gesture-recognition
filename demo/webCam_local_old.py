...
手势识别客户端Demo，支持27分类
    ...
    交互方式：
              1. 客户端向服务器端单次提交批量图片
              2. 服务器直接读取二进制流并分类，返回分类结果
    特点：无法设置超时时间，当服务器不稳定时容易崩溃
"""
...
class VideoCapture(object):
    ...
    def _predict(self):
        ...
        r = requests.post(self._server_address + "/old", data=allimg_bytes).text
        res = json.loads(r)

        print("All ok...[%.4f]" % (time.time() - start_time))
...
if __name__ == "__main__":
    cap = VideoCapture(device_index=0, server_address="http://127.0.0.1:5000")
    cap.run()